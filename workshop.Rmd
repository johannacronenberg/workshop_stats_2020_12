...
Dieses Dokument und eventuelle Updates finden Sie in einem [*github repository*](https://github.com/johannacronenberg/workshop_stats_2020_12).
...
```{r, eval = F}
install.packages(c("Rcpp", "rmarkdown", "gridExtra", "tidyverse", "rlang", "magrittr", 
                   "lme4", "lmerTest", "ez", "emmeans", "broom", "MuMIn"), dependencies = T)
```
...
Das heißt für die Normalverteilung mit Mittelwert Null und Standardabweichung Eins liegt eine Fläche von 0.95 zwischen -1.96 und 1.96. Oben hatten wir gesagt, dass die Fläche von 0.95 aber $\mu \pm2\sigma$ entspricht -- genauer hätten wir schreiben müssen $\mu \pm 1.96\sigma$.
...
Wir sehen also, dass das geschätzte Intercept bei 288.2 liegt und die Steigung bei -1.07. Die Steigung wird leider verwirrenderweise immer wie die $x$-Variable genannt, in diesem Fall also "Alter". Die Koeffizienten bedeuten folgendes: Bei einem Alter von Null Jahren ($x = 0$) liegt die mittlere Grundfrequenz wahrscheinlich bei ca. 288 Hz. Mit jeden weiteren Jahr ($x$ wird um 1 erhöht) sinkt die Grundfrequenz um 1.07 Hz. Indem wir Intercept und Slope in unsere Formel von vorhin einsetzen, können wir nun für alle möglichen Alter die entsprechende Grundfrequenz vorhersagen:
...
Der p-Wert kann hier wie folgt interpretiert werden: Wenn wir davon ausgehen, dass die tatsächliche Slope Null ist (das ist die Null-Hypothese), dann ist die hier beobachtete Steigung von -1.07 höchst unerwartet.
...
Mit einem Linear Mixed Effect Model (auch: Linear Mixed Effect Regression oder LMER) wird geprüft, ob eine kontinuierliche oder kategoriale Variable von einem oder mehreren unabhängigen Faktoren beeinflusst wird. Diese Art von Modellen wird als "gemischt" (*mixed*) bezeichnet, weil sowohl Faktoren, deren Einfluss auf die abhängige Variable für die Forschenden interessant sind (**Fixed Effects**), als auch Faktoren, deren Einfluss uninteressant sind (**Random Effects**), Teil des Modells sein können. Die Fixed Effects in einem Mixed Models können sowohl kategorial als auch numerisch sein, die Random Effects ausschließlich kategorial.
...
Bei der einfachen und multiplen linearen Regression haben wir ein Intercept berechnet und dann eine Slope pro unabhängiger Variable (pro Fixed Effect). Bei den gemischten Modellen wird dasselbe gemacht, aber jetzt schätzen wir zusätzlich noch für die Random Effects jeweils Intercepts und Slopes. Um die Konzepte **Random Intercept** und **Random Slope** zu erklären, konzentrieren wir uns hier erstmal auf `Vpn` als Random Effect.

## Random Intercepts vs. Random Slopes

### Random Intercepts

In der folgenden Abbildung sehen Sie die Dezibelwerte für die fünf weiblichen Probandinnen für die Vokale /a/ und /i/. Sie erinnern sich, dass mittels Treatment Coding der Vokal /a/ als Null und der Vokal /i/ als Eins interpretiert wird. Die Schätzung des Mittelwerts für /a/ für Frauen ist in unserem Mixed Model das Intercept. Die Kreuze in orange markieren die Dezibel-Mittelwerte pro Person und Vokal. Wenn wir uns jetzt aber die Mittelwerte für /a/ pro Sprecher anschauen, gibt es große Unterschiede (z.B. zwischen S10 und S6). Das ist ein Indikator dafür, dass es sinnvoll ist, pro Sprecher ein Intercept zu berechnen.

```{r}
ggplot(int %>% dplyr::filter(G == "f")) + 
  aes(x = V, y = db) + 
  geom_point() + 
  facet_wrap(~Vpn) + 
  geom_point(data = int %>% dplyr::filter(G == "f")%>% group_by(Vpn, V) %>% dplyr::summarise(db = mean(db)),
             color = "orange", size = 3, stroke = 2, shape = 4) + 
  geom_line(data = int %>% dplyr::filter(G == "f")%>% group_by(Vpn, V) %>% dplyr::summarise(db = mean(db)),
            aes(x = V, y = db, group = Vpn),
             lty = "dashed")
```

In der R Notation wird ein Random Intercept so geschrieben: `(1 | Vpn)`. Hierbei steht `1` für Intercept, ausgesprochen bedeutet die Formel so viel wie "schätze ein Intercept in Relation zu den individuellen Versuchspersonen". In der Praxis berechnet ein Mixed Model mit einem solchen Random Intercept allerdings nicht ein Intercept pro Person, sondern die Standardabweichung der Versuchspersonen-Intercepts vom geschätzten "allgemeinen" Intercept über alle Datenpunkte.

### Random Slopes

In der obigen Abbildung sehen Sie neben den Mittelwerten für /a/ auch die für /i/, und eine gestrichelte Linie, die beide verbindet. Diese stellt hier sozusagen die ideale Regressionslinie pro Versuchsperson dar. Da die Linien ganz unterschiedlich steil verlaufen, können wir erahnen, dass die einzelnen Versuchspersonen unterschiedliche Slopes haben. Sprecherin S7 mach z.B. kaum einen Lautstärke-Unterschied zwischen /a/ und /i/ (ihre Slope ist ca. 0), während Sprecherin S6 das /a/ deutlich lauter produziert als das /i/ (ihre Slope ist negativ). Hier scheint es also sinnvoll zu sein, auch die Slopes sprecherspezifisch zu berechnen.

Allgemein wird davon ausgegangen, dass auch ein Random Intercept geschätzt werden soll, wenn wir Random Slopes schätzen. Daher ist die Formel für Random Slopes `(1 + V | Vpn)`, also "schätze ein Intercept und eine Slope für Vokal in Relation zu den individuellen Versuchspersonen". Häufig wird anstatt dieser Notation allerdings die Kurzform `(V | Vpn)` verwendet, da für die Funktion dann klar ist, dass sowohl Random Slope als auch Random Intercept berechnet werden soll. Auch hier wird wieder die Abweichung von der "allgemeinen" Slope geschätzt, und nicht wirklich eine Slope pro Proband.

Vielleicht fragen Sie sich jetzt, warum wir die ganze Zeit den Fixed Effect `G` ignorieren. Warum z.B. interessieren wir uns nicht für `(1 + V + G | Vpn)`? Das liegt daran, dass die Stufen des Faktors `G` nicht pro Versuchsperson variieren, denn die Versuchspersonen sind *entweder* männlich *oder* weiblich. Hier gibt es also gar keine Variation, die wir durch die entsprechende Random Slope herausrechnen könnten. Für die Stufen des Faktors `V` hingegen liegen jeweils Werte für die Versuchspersonen vor (die haben sowohl /a/ als auch /i/ produziert).

*(Im Folgenden benutzen wir eine Random Effects Notation für `Wort`, die außer Acht lässt, dass Wort auch mit Vokal variiert, weil anscheinend sowohl /i/ als auch /a/ in jedem Wort vorkamen. Wir lassen diesen Faktor aus didaktischen Gründen weg, um die Sache nicht komplizierter zu machen als sie ist.)*

## LMER in R

Das klassische Paket, das für LMERs benutzt wird, heißt `lme4`. Wir nutzen hier stattdessen `lmerTest`, das aber einfach ein Wrapper für `lme4` ist. Die Funktion für das Berechnen eines Mixed Models heißt `lmer()`. Unsere vollständige Formel enthält den üblichen ersten Teil für die Fixed Effects inklusive Interaktion, und dann die Random Effects für `Vpn` und `Wort`. Dieses Mal schauen wir uns die Ergebnisse des Mixed Models mit der Funktion `summary()` an (es gibt aber auch ein Paket namens [`broom.mixed`](https://cran.r-project.org/web/packages/broom.mixed/vignettes/broom_mixed_intro.html), das die Funktionen `tidy()`, `augment()` und `glance()` für LMERs bereitstellt).

```{r}
int.lmer <- lmer(db ~ G * V + (1 + V | Vpn) + (1 + G | Wort), data = int, REML = F)
summary(int.lmer)
```

Hier wird uns zuerst angezeigt, was genau für ein Modell berechnet und welche Formel dafür verwendet wurde. `REML` steht für *Restricted Maximum Likelihood*. Indem wir der Funktion das Argument `REML = F` gegeben haben, wird nun stattdessen eine echte Maximum Likelihood Schätzung der gewünschten Parameter ausgeführt (für eine Übersicht zum Thema REML, s. Winter (2020)). Es folgen die üblichen Kennzahlen für die Residuals. Neu für uns ist die Tabelle für die Random Effects, altbekannt ist die Tabelle für die Fixed Effects. Zuletzt werden uns hier die Korrelationen zwischen den Fixed Effects angezeigt, aber auch damit werden wir uns hier nicht beschäftigen. Stattdessen schauen wir uns zuerst die Fixed Effects nochmal genauer an.

### Fixed Effects

![](img/fixed_effects.png){width=70%}

Diese Tabelle kommt uns bekannt vor. Wir sehen die Schätzung für das Intercept und dann die Schätzungen für die Slopes `Gm`, `Vi` und die Interaktion `Gm:Vi`. Der geschätzte dB-Mittelwert für das /a/ von Frauen liegt bei 48.2 dB. Männer sprechen offenbar deutlich lauter, denn die Slope für `Gm` ist hoch und positiv. Das heißt das /a/ von Männern liegt bei ca. `48.2 + 40.2 = 88.4` dB. Der Vokal /i/ wird hingegen leiser produziert als /a/, denn die Slope ist negativ mit -10 dB. Zuletzt sehen wir dass für das /i/ von Männern nochmal 20.2 dB abgezogen werden müssen, das heißt der geschätzte Mittelwert für das /i/ von Männern liegt bei `48.2 + 40.2 + (-10.0) + (-20.2) = 58.2` dB. 

Dazu werden uns die Standard Errors gegeben und die Ergebnisse der $t$-Statistik. Hier sehen wir, dass das Geschlecht einen signifikanten Einfluss auf die produzierte Lautstärke hat und dass es eine signifikante Interaktion zwischen Geschlecht und Vokal gab. Der Vokal hatte hingegen keinen signifikanten Einfluss auf die Lautstärke.

*Da es eine signifikante Interaktion gibt, würden wir hier unter realistischen Bedingungen noch post-hoc Tests mit `emmeans` ausführen.*

### Random Effects

![](img/random_effects.png){width=70%}

Nun schauen wir uns die Random Effects genauer an. Zuerst sehen wir, dass für den Random Factor `Vpn` die Varianz und Standardabweichung der Intercepts und Slopes mit Relation zum Vokal geschätzt wurden. Die Standardabweichung für die sprecherspezifischen Intercepts liegt hier bei 14.7 dB, d.h. die Sprechervariation um das "allgemeine" Intercept von 48.2 dB liegt bei $\pm 14.7$ dB, was in Relation recht viel ist. Die Sprechervariation um die "allgemeine" Slope für `Vi` von -10 dB liegt bei 10.4 dB. Zusätzlich sehen wir dann aber in der Spalte `Corr` den Korrelationskoeffizienten $r$ für die Korrelation zwischen dem Random Intercept und der Random Slope. Da die Korrelation hier negativ ist, bedeutet das, dass Probanden mit einem höheren Intercept eine steilere negative Slope für /i/ haben. Wenn also jemand das /a/ besonders laut produziert, produzierten sie auch ein leiseres /i/ (Achtung: wir beschreiben hier keine Kausalität, sondern eine Korrelation!).

Das heißt also, durch die Formel `(1 + V | Vpn)` wurden drei Parameter geschätzt: Ein Random Intercept, eine Random Slope, und die Korrelation zwischen den beiden. Seien Sie daher vorsichtig, wenn Sie ihre eigenen Modelle konzipieren: Je mehr komplexe Random Effects Formeln Sie verwenden, desto mehr Parameter müssen geschätzt werden, und das kann zu sehr sehr langen Rechenzeiten und Konvergenzproblemen führen.

Apropos Konvergenzprobleme: Schauen wir uns den Random Effect `Wort` an. Hier haben wir ebenfalls ein Random Intercept berechnet, für das eine Standardabweichung von 14.7 dB geschätzt wurde. So weit so gut. Dann sehen wir aber, dass die Random Slope eine extrem kleine Standardabweichung hat mit nur 0.3 dB. Das bedeutet, dass das Wort gar nicht mit dem Geschlecht der Versuchspersonen variiert. Das Mixed Model hat aber trotzdem versucht, die Random Slope und die Korrelation zwischen Random Intercept und Random Slope zu schätzen -- und ist gescheitert. Das sehen Sie daran, dass $r = -1$ (auch bei $r = 1$), denn eine perfekte Korrelation gibt es nicht. Die 1 oder -1 zeigt, dass die Korrelation nicht geschätzt werden konnte. Daher rührt auch mit großer Wahrscheinlichkeit der Fehler, der uns bei der Berechnung des Modells angezeigt wurde:

![](img/fehler.png){width=70%}

## Modell vereinfachen

Für solche Fälle gibt es die `step()` Funktion. Diese Funktion schaut sich alle Fixed und Random Effects Strukturen im Modell an, und berechnet, welche davon signifikant sind und damit etwas Essentielles zum Modell beitragen und welche nicht. Die Faktoren, die nichts beitragen, werden eliminiert. So bleibt am Ende ein Modell, dass nur die statistisch relevanten Faktoren mit einbezieht. Das heißt aber im übrigen *nicht*, dass Sie nicht trotzdem Faktoren im Modell behalten können, die nicht signifikant sind! Hier sehen Sie das Ergebnis von `step()`:
...
Es kommt vor, dass die Funktion mehrfach den obigen Fehler wirft, und zwar immer dann, wenn sie ein neues Modell mit einer angepassten Formel berechnet und der Fehler immer noch da ist. Zuerst sehen wir im Ergebnis die *Backwar reduced random-effect table*. Die *backward reduction* ist ein Prozess, bei dem zuerst das komplexeste Modell ausprobiert wird und dann werden nach und nach die nicht signifikanten Terme entfernt. Hier sehen wir, dass die Random Slope `G` in dem Random Effect `(1 + G | Wort)` nicht signifikant ist, und deshalb entfernt wurde. Es bleibt daher nur noch das Random Intercept `(1 | Wort)` übrig. Auch dieses wurde nochmal evaluiert, in Zeile 3, und für wichtig befunden. Auch die Random Slope `V` in `(1 + V | Vpn)` kann bleiben. Es folgt dieselbe Prozedur für die Fixed Effects.

Zuletzt zeigt uns `step()` an, welches Modell es schließlich ausgewählt hat, nämlich:

`db ~ G + V + (1 + V | Vpn) + (1 | Wort) + G:V`

Unsere Fixed Effects werden hier ausgeschrieben als `G + V + ... G:V`. Random Intercept und Slope für Versuchspersonen sind unverändert, und für den Random Factor `Wort` wird jetzt nur noch ein Intercept geschätzt. Mit der Funktion `get_model()` können wir uns wieder die Ergebnisse des vereinfachten Modells anzeigen lassen:

```{r}
int.lmer.new <- get_model(int.step)
summary(int.lmer.new)
```

Zuerst bemerken wir, dass der Fehler nicht mehr auftaucht. In der Random Effects Tabelle erscheint jetzt nur noch das Random Intercept für Wort, nicht mehr die Random Slope. Der Rest ist unverändert. Nun führen wir noch die paarweisen Vergleiche mit `emmeans` aus, weil die Interaktion `Gm:Vi` signifikant ist:

```{r}
emmeans(int.lmer.new, pairwise ~ V | G)$contrasts
```

```{r}
emmeans(int.lmer.new, pairwise ~ G | V)$contrasts
```

Wir berichten also folgendes:

**Es wurde eine Linear Mixed Effect Regression durchgeführt mit der Lautstärke in Dezibel als abhängiger Variable, sowie mit den Fixed Effects für Geschlecht und Vokal inklusive der Interaktion zwischen den Faktoren, und den Random Effects für Versuchspersonen und Wörter. Für Versuchspersonen wurden sowohl Random Intercept als auch Random Slope und für Wörter nur das Random Intercept geschätzt. Das Modell ergab einen signifikanten Einfluss von Geschlecht ($t$[9.7] = 4.3, $p$ < 0.01) auf die Lautstärke. Es gab zusätzlich eine signifikante Interaktion zwischen Geschlecht und Vokal ($t$[10.0] = 3.1, $p$ < 0.05). Post-hoc t-Tests zeigten signifikante Differenzen zwischen /a/ und /i/ für Männer ($t$[12.5] = 5.8, $p$ < 0.001), aber nicht für Frauen, und signifikante Differenzen zwischen Männern und Frauen für /a/ ($t$[11.2] = 4.0, $p$ < 0.01), jedoch nicht für /i/.**

## Gütekriterien für das Modell

Sie haben vielleicht bemerkt, dass uns die `summary()` Funktion kein $R^2$ ausgegeben hat, wie das zuvor bei den Regressionen mit `lm()` der Fall war. Das liegt daran, dass jetzt separat für Fixed und Random Effects evaluiert werden muss, wie viel Varianz in den Daten sie jeweils beschreiben. Das Paket `MuMIn` stellt die Funktion `r.squaredGLMM()` bereit, die wir auf unser Mixed Model anwenden können:

```{r}
r.squaredGLMM(int.lmer.new)
```

Hier werden uns jetzt folgerichtig zwei $R^2$-Werte angezeigt. Der erste, `R2m` (das steht für **marginal $R^2$**), ist die Proportion der Varianz in den Daten, die durch die Fixed Effects beschrieben wird, hier also 46%. Der zweite, `R2c` (für **conditional $R^2$**), ist die Proportion der Varianz in den Daten, die durch die Fixed Effects und Random Effects *zusammen* beschrieben wird. Mit 99.5% haben wir also quasi die gesamte Varianz in den Daten beschrieben! (Solch hohe Werte werden Sie in echten linguistischen Studien niemals sehen). Hieraus können wir ableiten, dass die Random Effects $99.5 - 46.1 = 53.4$ Prozent der Varianz beschreiben. Es war also für den Datensatz `int` sehr wichtig, dass wir die Random Effects im Modell hatten.

Im Ergebnis des Modells fehlte außerdem ein Indikator für die Güte des Modells. Dies wurde uns bei `lm()` durch den F-Test angezeigt. Bei LMERs wird die Güte mit **Likelihood Ratio Tests** evaluiert. Dabei wird das verwendete Mixed Model mit einem anderen Mixed Model verglichen, bei dem ein Faktor eliminiert wurde. So kann eingeschätzt werden, ob durch das Weglassen eines Faktors die Güte des Modells sinkt oder gleich bleibt. Wie Likelihood Ratio Tests durchgeführt werden, geht jedoch über diesen Kurs hinaus.
